    ...
    <div className="flex flex-col h-full bg-gray-950 text-white overflow-y-auto">
      <div className="bg-gray-900 px-8 pt-10 pb-6 border-b border-gray-800">
        ...
        <div className="mt-5">
          ...
          <div className="h-3 bg-gray-800 rounded-full overflow-hidden">
            <div className="h-full bg-red-600 rounded-full transition-all duration-700" style={{ width: `${(totalCompleted / 18) * 100}%` }} />
          </div>
        </div>
      </div>
      ...
      {nextDay && (
        <div className="mx-6 mt-6">
          <button onClick={() => onStartDay(nextDay)} className="w-full bg-red-600 rounded-2xl px-6 py-5 flex items-center justify-between active:bg-red-700 transition-all">
            <div className="text-left">
              <div className="text-xs font-semibold text-red-200 uppercase tracking-wider mb-1">Up Next</div>
              <div className="text-xl font-bold text-white">Week {nextDay.week} · Day {nextDay.day}</div>
              <div className="text-red-200 text-sm mt-0.5">{nextDay.exercises.length} exercises</div>
            </div>
                ...
                {weekDays.map((dayData) => {
                  ...
                  return (
                    <button key={dayData.id} onClick={() => onStartDay(dayData)}
                      className={`rounded-2xl border text-left transition-all active:scale-95 overflow-hidden ${completed ? 'bg-green-950 border-green-800' : 'bg-gray-900 border-gray-800 active:border-red-600'}`}>
                      <div className="relative h-28 bg-gray-800 overflow-hidden">
                        ...
                        {completed && <div className="absolute inset-0 bg-green-900/70 flex items-center justify-center"><span className="text-4xl">✅</span></div>}
                      ...
                      <div className="px-4 py-3">
                        ...
                        <div className={`mt-3 rounded-lg py-2 text-center text-xs font-bold ${completed ? 'bg-green-800 text-green-200' : 'bg-red-600 text-white'}`}>{completed ? 'Done' : 'Start'}</div>
                      </div>
                    </button>
                  )
                })}
