...
export default function WorkoutScreen({ dayData, onBack, onComplete, logSet, getLastWeight }) {
  ...
  function advanceAfterRest() {
    ...
    if (isLastSet) {
      ...
      else { setExerciseIdx(i => i + 1); setSetIdx(0) }
    } else {
      setSetIdx(s => s + 1)
    }
  }
  ...
  function skipRest() { clearInterval(timerRef.current); advanceAfterRest() }

  if (phase === 'rest') {
    return <RestScreen seconds={restRemaining} total={currentEx.rest} nextExercise={isLastSet && !isLastExercise ? dayData.exercises[exerciseIdx + 1]?.name : null} onSkip={skipRest} />
  }
  ...
  return (
    <div className="flex flex-col h-full bg-gray-950 text-white overflow-hidden">
      ...
      <div className="relative flex-1 bg-black min-h-0 overflow-hidden">
        {previewImg && <img src={previewImg} className="w-full h-full object-cover object-top" onError={e => { e.target.style.display='none' }} />}
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
        <button onClick={() => setShowDesc(d => !d)} className="absolute top-3 right-3 bg-black/50 backdrop-blur-sm rounded-full px-3 py-1.5 text-xs font-semibold text-white/80 active:bg-black/70">{showDesc ? 'Hide' : 'How to'}</button>
        {exData?.muscles && <div className="absolute top-3 left-3 bg-black/50 backdrop-blur-sm rounded-full px-3 py-1.5 text-xs text-white/80">{exData.muscles}</div>}
        {showDesc && exData?.description && <div className="absolute inset-0 bg-black/85 flex items-center justify-center p-8"><p className="text-white text-base leading-relaxed text-center">{exData.description}</p></div>}
        <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-2">
          {Array.from({ length: currentEx.sets }).map((_, i) => (
            <div key={i} className={`w-3 h-3 rounded-full border-2 border-white/40 transition-all ${i < setIdx ? 'bg-green-500 border-green-500' : i === setIdx ? 'bg-red-500 border-red-500' : 'bg-transparent'}`} />
          ))}
        </div>
      </div>
      ...
      <div className="flex gap-4 px-6 py-4 bg-gray-950 flex-shrink-0">
        ...
        <div className="flex-1 bg-gray-900 rounded-2xl border border-gray-800 p-4">
          <div className="text-xs text-gray-500 uppercase tracking-wide mb-2 text-center">{isTimed ? 'Seconds Done' : 'Reps Done'}</div>
          <div className="flex items-center gap-2">
      ...
      <div className="px-6 pb-2 flex-shrink-0">
        <button onClick={handleLogSet} disabled={justLogged}
          className={`w-full py-5 rounded-2xl text-lg font-bold transition-all active:scale-95 ${justLogged ? 'bg-green-600' : 'bg-red-600 active:bg-red-700'}`}>
          {justLogged ? '✓ Logged!' : isLastSet && isLastExercise ? 'Finish Workout' : 'Log Set'}
        </button>
      </div>

      <div className="flex items-center justify-between px-6 pb-6 pt-1 flex-shrink-0">
        <button onClick={() => { setExerciseIdx(i => Math.max(0, i-1)); setSetIdx(0) }} disabled={exerciseIdx === 0}
          className="flex items-center gap-1 text-gray-500 disabled:opacity-30 active:text-white py-2 px-3">
          <span className="text-xl">‹‹</span><span className="text-sm">Prev</span>
        </button>
        <div className="flex gap-1.5">
          {dayData.exercises.map((_, i) => (
            <div key={i} className={`rounded-full transition-all ${i < exerciseIdx ? 'w-2 h-2 bg-green-500' : i === exerciseIdx ? 'w-3 h-3 bg-red-500' : 'w-2 h-2 bg-gray-700'}`} />
          ))}
        </div>
        <button onClick={() => { setExerciseIdx(i => Math.min(dayData.exercises.length-1, i+1)); setSetIdx(0) }} disabled={exerciseIdx === dayData.exercises.length - 1}
          className="flex items-center gap-1 text-gray-500 disabled:opacity-30 active:text-white py-2 px-3">
          <span className="text-sm">Next</span><span className="text-xl">››</span>
        </button>
      </div>
    </div>
  )
...
function StepBtn({ label, onPress }) {
  return (
    <button onClick={onPress} className="w-14 h-14 rounded-xl bg-gray-800 text-white text-2xl font-bold flex items-center justify-center active:bg-gray-700 active:scale-95 transition-all flex-shrink-0">{label}</button>
  )
}
...
function RestScreen({ seconds, total, nextExercise, onSkip }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full bg-gray-950 text-white px-8">
      ...
      <button onClick={onSkip} className="bg-gray-800 text-white px-10 py-4 rounded-2xl text-lg font-semibold active:bg-gray-700 active:scale-95 transition-all">Skip Rest →</button>
    </div>
  )
}
